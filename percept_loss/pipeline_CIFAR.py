...
if __name__ == '__main__':
    ...
    for run in tqdm(all_runs, desc='All Runs'):
        ...
        saver = train_saver(epochs, loss, network, batch_size, data_percent, dataset='CIFAR') # saver
        loss_func = LOSS[loss]()
        network_func = CIFAR_AUTOENCODERS[network]()
        train(network_func, loss_func, epochs, device, saver, data_percent, pre_loaded_images)
